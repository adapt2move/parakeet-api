...
Choose one of `colocated.yaml` and `distributed.yaml`. Both create namespace `parakeet`, a ClusterIP API service and a 4 GiB PVC using the cluster's default storage class. The colocated variant has two containers in one Pod. In the distributed variant, scale `deployment/parakeet-worker` to add processing capacity.
...
Both containers run as UID/GID 10001 with a read-only root filesystem, dropped capabilities and independent probes. API uses a PVC for SQLite and audio; worker scratch stays on disk in its own `emptyDir`. API upgrades use `Recreate` to avoid concurrent ownership of SQLite. Do not scale the API above one replica. API availability during updates is a deliberate limit of this first version.
...
Each additional worker can consume up to 3.5 CPU and 6 GiB RAM. Reserve resources accordingly. Use the smaller limits from `compose.yaml` when testing on a Mac. To disable durable job storage, replace the `data` PVC volume with disk `emptyDir`; Pod replacement then loses jobs and results.

Before a production rollout, pin image digests, choose the storage class and public base URL, and add network rules appropriate to the cluster. Allow worker-to-API port 8080. Keep the worker health port private and block `/internal/*` at any external ingress. An API-key holder can access all jobs; this deployment is for a shared internal trust boundary.
